...
use wgpu::{
    BindGroup, BindGroupLayout, BlendState, Buffer, BufferDescriptor, BufferSize, BufferUsages,
    ColorTargetState, ColorWrites, CompareFunction, DepthStencilState, Device, FragmentState,
    IndexFormat, MultisampleState, PipelineLayoutDescriptor, RenderPass, RenderPipeline,
    RenderPipelineDescriptor, ShaderModuleDescriptor, ShaderSource, VertexState,
};

use crate::asset::AssetManagerRc;
use crate::model::{
    InstGridBuf, InstPhongColorBuf, InstShaderImplType, InstShaderSize, InstShaderType,
    InstSimpleColorBuf, InstWindowBuf, Mesh,
};
...
impl ModelRegistry {
    ...
    pub fn create<F: ModelFactory>(&mut self, factory: F) -> Rc<F::Model> {
        ...
        let model_info = self
            .model_infos
            .entry(F::get_name().to_string())
            .or_insert_with(|| {
                let mesh = F::get_mesh(Arc::clone(&self.asset_mgr), device);
                let submeshes = mesh.get_submeshes();

                let inst_sh_impls = submeshes
                    .iter()
                    .map(|submesh| submesh.get_inst_sh_type().create_impl())
                    .collect();
                let visibles = Rc::new(RefCell::new(
                    iter::repeat_with(HashSet::new)
                        .take(submeshes.len())
                        .collect(),
                ));
        ...
        let model = Rc::new(factory.create(
            handle,
            device,
            &mut model_info.inst_sh_impls,
...
impl ModelRenderer {
    fn new(
        ...
        let render_infos = model_infos
            ...
            .map(|model_info| {
                ...
                let inst_sh_infos = mesh
                    .get_submeshes()
                    .iter()
                    .zip(model_info.inst_sh_impls.iter())
                    .map(|(submesh, inst_sh_impl)| {
                        ...
                        let (bg_layout_opt, pipeline_layout) = pipeline_layouts
                            ...
                            .or_insert_with(|| {
                                ...
                                let pipeline_layout = Rc::new(device.create_pipeline_layout(
                        ...
                        let pipeline = pipelines
                            .entry((
                                pipeline_layout_key,
                                vertex_sh_type.clone(),
                                inst_sh_type.clone(),
                                primitive_st_type.clone(),
                            ))
                            .or_insert_with(|| {
                                let shader = shaders
                                    .entry((vertex_sh_type.clone(), inst_sh_type.clone()))
                                    .or_insert_with(|| {
                                        let name = format!(
                                            "/shader/{}_{}.wgsl",
                                            vertex_sh_type.get_name(),
                                            inst_sh_type.get_name()
                                        );
                                        ...
                                        let source = asset_file
                                            ...
                                            .replace(
                                                "#VIEW_INDEX_DEF#",
                                                output_info.get_view_index_def(),
                                            )
                                            .replace(
                                                "#VIEW_INDEX_VAL#",
                                                output_info.get_view_index_val(),
                                            );

                                        Rc::new(device.create_shader_module(
                                ...
                                Rc::new(device.create_render_pipeline(&RenderPipelineDescriptor {
                                    ...
                                    vertex: VertexState {
                                        ...
                                        buffers: &[vertex_sh_type.get_layout(), inst_sh_layout],
                                    },
    ...
    pub fn render(&self, render_pass: &mut RenderPass) {
        ...
        for render_info in &self.render_infos {
            ...
            for (inst_index, ((visible_model_indexes, inst_sh_info), submesh)) in visibles
                .iter()
                .zip(render_info.inst_sh_infos.iter())
                .zip(mesh.get_submeshes().iter())
                .enumerate()
            {
                let visible_model_indexes_len: u32 =
                    visible_model_indexes.len().try_into().unwrap();
                ...
                if visible_model_indexes_len > 0 {
                    ...
                    let mut inst_buf_view = queue
                        .write_buffer_with(inst_buf, 0, BufferSize::new(total_size).unwrap())
                        .unwrap();
                    ...
                    if !mesh_bound {
                        ...
                        render_pass
                            .set_index_buffer(mesh.get_index_buf().slice(..), IndexFormat::Uint16);
                        mesh_bound = true;
                    }
                    ...
                    render_pass.draw_indexed(
                        submesh.get_indices(),
                        submesh.get_base_vertex(),
                        0..visible_model_indexes_len,
                    );
