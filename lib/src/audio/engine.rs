...
type WorkerMessage = (
    Box<dyn FnOnce() -> Box<dyn AudioSource + Send> + Send>,
    AudioPosAtomic,
);
...
struct Inner {
    // TODO: We have only a single field, keep struct?
...
impl AudioEngine {
    ...
    pub fn new() -> Self {
        ...
        let device = host
            ...
            .expect("Unable to determine default audio device");
        ...
        let all_ranges: Vec<_> = device
            .supported_output_configs()
            .expect("Unable to determine supported formats")
            .filter(|range| {
                range.channels() == CHANNELS && range.sample_format() == SampleFormat::F32
            })
            .collect();

        let mut ranges: Vec<_> = all_ranges
            .iter()
            .filter(|range| {
                range.min_sample_rate().0 <= MIN_SAMPLE_RATE
                    && range.max_sample_rate().0 >= MIN_SAMPLE_RATE
            })
            .collect();
        let (range, sample_rate) = if !ranges.is_empty() {
            ...
            ranges = all_ranges
                .iter()
                .filter(|range| range.min_sample_rate().0 >= MIN_SAMPLE_RATE)
                .collect();
        ...
        let mut config: StreamConfig = range
    ...
    fn build_stream(device: &Device, config: &StreamConfig, inner_mutex: InnerMutex) -> Stream {
        ...
        let mut source_buf = Vec::from_iter(iter::repeat_n(
            0.0,
            CHANNELS as usize * sample_rate as usize,
        ));
    ...
    pub fn add<T: AudioInput + Send + 'static>(&self, input: T) -> AudioTimestamp {
        ...
        self.worker_tx
            .send((
                Box::new(move || Box::new(input.build(CHANNELS, sample_rate))),
                Arc::clone(&pos_atomic),
            ))
            .unwrap();
